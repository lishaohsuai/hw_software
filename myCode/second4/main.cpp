...
void dfs6o(int head, int cur, int depth) 
{
    ...
    for(i = 0; i < Gout[cur].size(); ++i) {
        ...
        if (depth >= 4 && vis1[v] != tmpFlag) // 如果 vis1 没有遍历到那么结果就弹出 就是正向遍历
        {
            tmpDist.pop_back();
        ...
        if(vis2[v] == tmpFlag && depth >= 2) {   // 存在路径得到一条结果 减少最后一层遍历
            ...
            for(j = 0; j< tmpDist.size(); j++){
                if(j == 0){
                    unsigned int leftMoney = tmpDist[tmpDist.size() - 1];
                    unsigned int rightMoney = tmpDist[0];
                    ...
                    unsigned int leftMoney = tmpDist[j - 1];
                    unsigned int rightMoney = tmpDist[j];
                    double scale = rightMoney * 1.0 / leftMoney;
                    if(!(scale >= 0.2 && scale <= 3)){
                        flagRecv = false;
                        break;
