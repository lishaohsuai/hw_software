...
public:
    ...
    typedef std::pair<unsigned int, unsigned int> pairMoney;
    std::map<pairMoney, unsigned int> money;
    ...
    std::vector<double> moneys; // 存储金额 C
    ...
    void dfs(int head, int cur, int depth, std::vector<int> &path) {
        ...
        for (int &v : G[cur]) {
            ...
            // 检查判断条件 
            // 循环转账的前后路径的转账金额浮动，不能小于0.2，不能大于3。
            // 如账户A转给账户B 转X元，账户B转给账户C转Y元， 那么进行循环检测时，要满足`0.2 <= Y/X <= 3`的条件。
                ...
                for(int i=0; i<tmp.size(); i++){
                    if(i == 0) {
                        unsigned int leftMoney = money[std::make_pair(tmp[tmp.size()-1], tmp[0])];
                        unsigned int rightMoney = money[std::make_pair(tmp[0], tmp[1])];
                        ...
                        unsigned int leftMoney = money[std::make_pair(tmp[tmp.size()-2], tmp[tmp.size()-1])];
                        unsigned int rightMoney = money[std::make_pair(tmp[tmp.size()-1], tmp[0])];
                        double scale = rightMoney * 1.0 / leftMoney;
                        if(!(scale >= 0.2 && scale <= 3) ){
                            flagRecv = false;
                            break;
                        ...
                        unsigned int leftMoney = money[std::make_pair(tmp[i-1], tmp[i])];
                        unsigned int rightMoney = money[std::make_pair(tmp[i], tmp[i+1])];
                        double scale = rightMoney * 1.0 / leftMoney;
                        if(!(scale >= 0.2 && scale <= 3) ){
                            flagRecv = false;
                            break;
...
int main()
{
    ...
    // std::string outputFile = "../std/testout/second.txt";
